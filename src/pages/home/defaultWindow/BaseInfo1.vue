  ...
  <div class="panel-box">
    ...
    <div class="panel-content scroll-box">
      <div style="height: 100%">
        <div class="button" @click="openPop1">打开pop1</div>
        <div class="button" @click="openPop2">打开pop2</div>
        <!-- <div class="panel-content-header" v-for="(row, index) in list" :key="index">
          <div class="header-list" v-for="(item, key) in row" :key="key">
            <div class="list-content" :title="item.hoverTitle">
              <span class="total">{{ item.dataCount }}</span>
              <span>{{ item.unit }}</span>
            </div>
...
import { WindowSideI } from "@/interface";
...
const { openPopWindow } = useWindowStore();
defineProps<ProsI>();
const list = ref<any[]>([[], []]);
onMounted(() => {
  init();
...
let count = 0
function openPop1() {
  openPopWindow("PopWindow1", {count: Math.random()});
}
function openPop2() {
  openPopWindow("PopWindow2", {count: count++});
}
